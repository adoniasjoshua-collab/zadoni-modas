...
(async()=>{
 ...
 for(const width of [320,375,390,768,1024,1440]){
  ...
  for(const route of ['/','/colecao/','/produto/vestido-midi-aurora/']){
   ...
   if(route==='/'){
    assert.equal(await page.locator('#catalog-grid .product-card').count(),17);
 ...
 await page.selectOption('[name=categoria]','vestidos');assert.equal(await page.locator('#catalog-grid .product-card').count(),7);
 await page.selectOption('[name=tamanho]','P');assert.equal(await page.locator('#catalog-grid .product-card').count(),5);
 await page.locator('[type=reset]').click();await page.waitForTimeout(50);assert.equal(await page.locator('#catalog-grid .product-card').count(),17);
 ...
 await page.locator('[type=reset]').click();await page.waitForTimeout(50);assert.equal(await page.locator('#catalog-grid .product-card').count(),17);assert.equal(await page.locator('h1').innerText(),'Nossa coleção');
 await page.goto(origin+'/colecao/?q=brisa');assert.equal(await page.locator('#catalog-grid .product-card').count(),1);
 await page.goto(origin+'/colecao/?tamanho=P');assert.equal(await page.locator('#catalog-grid .product-card').count(),15);
 await page.locator('.product-image').first().click();assert(page.url().includes('/produto/'));assert.equal(await page.locator('[data-related] .product-card').count(),2);
 ...
 assert(message.includes('Vestido Longo Serena'));assert(message.includes('160,00'));assert(message.includes('Tamanho: GG'));assert(message.includes('Cor: Preto'));assert(message.includes('/produto/vestido-longo-serena/'));
 ...
 await page.locator('.product-image').first().click();assert(page.url().includes('/preview/produto/'));
